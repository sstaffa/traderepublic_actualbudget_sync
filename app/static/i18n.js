(function () {
  const translations = {
    de: {
      ...
      'filters.body': 'Ausgeschlossene Event-Typen werden gar nicht erst nach Actual importiert. Konfiguration ausschließlich über TR_EXCLUDED_EVENT_TYPES in der .env. Unbekannte, neue Event-Typen werden immer importiert.',
      'filters.reload': 'Neu laden',
      'filters.excluded': 'ausgeschlossen',
      'filters.sourceLive': 'Live aus {path} — Änderungen wirken ohne Neustart, nach Klick auf „Neu laden“.',
      'filters.sourceStatic': 'Aus den Umgebungsvariablen beim Containerstart — Änderungen wirken erst nach einem Neustart des Containers.',
    ...
    en: {
      ...
      'filters.title': 'Event type filter', 'filters.body': 'Excluded event types are never imported into Actual. Configured exclusively through TR_EXCLUDED_EVENT_TYPES in the .env file. New, unknown event types are always imported.', 'filters.reload': 'Reload', 'filters.excluded': 'excluded', 'filters.sourceLive': 'Live from {path} - changes apply without a restart, after clicking "Reload".', 'filters.sourceStatic': 'From the environment captured at container start - changes only apply after restarting the container.', 'filters.unknownTitle': 'Unknown (set in .env but not in the catalogue)', 'filters.loadFailed': 'Could not load filters',
